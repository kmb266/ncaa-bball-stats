...
def get_season_start_end_datetimes(league, season_year):
    ...
    if league == "mlb":
        ...
        # KB - I hardcoded in the start date because the randome thing was taking too long.. and I know games start in Nov and end in April
        return datetime.datetime(season_year,10,31, tzinfo=pytz.timezone("US/Eastern")).astimezone(pytz.utc), datetime.datetime(season_year+1,4,20, tzinfo=pytz.timezone("US/Eastern")).astimezone(pytz.utc)
    elif league == "wnba":
...
def get_filename(cached_json_path, league, data_type, url, game_id=None):
    ...
    if game_id: filename = dir_path + game_id
...
def update_cached_file(url, chached_path=None, game_id=None):
    """ Update an ESPN JSON data or HTML BeautifulSoup cache if current date is in season """
    pass

def get_url(url, cached_path=None, game_id=None):
    ...
    return get_cached_url(url, league, data_type, cached_path, {}, game_id)

def get_cached_url(url, league, data_type, cached_path, headers={}, game_id=None):
    """ get_url helper if want to specify the league and datatype (for non espn.com links) """
    if cached_path:
        filename = get_filename(cached_path, league, data_type, url, game_id)
